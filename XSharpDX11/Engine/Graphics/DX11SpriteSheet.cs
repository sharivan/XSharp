...
public class DX11SpriteSheet : SpriteSheet
{
    public new static DX11Engine Engine => (DX11Engine) SpriteSheet.Engine;

    public new DX11Texture CurrentTexture => (DX11Texture) base.CurrentTexture;

    internal DX11SpriteSheet(bool disposeTexture = false, bool precache = false)
        : base(disposeTexture, precache)
    {
    }

    internal DX11SpriteSheet(ITexture texture, bool disposeTexture = false, bool precache = false)
        : base(texture, disposeTexture, precache)
    {
    }

    internal DX11SpriteSheet(string imageFileName, bool precache = false)
    ...
    public override Frame AddFrame(MMXBox boudingBox, MMXBox hitbox)
    {
        ...
        if (Precache)
        {
            ...
            else
            {
                ...
                using (var dstDS = new DX11DataStream(dstRect.DataPointer, width1 * height1 * sizeof(byte), true, true))
                {
                    using var srcDS = new DX11DataStream(srcRect.DataPointer, srcWidth * srcHeight * sizeof(int), true, true);
