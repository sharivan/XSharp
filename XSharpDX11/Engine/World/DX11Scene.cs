...
public class DX11Scene : Scene
{
    new public static DX11Engine Engine => (DX11Engine) Scene.Engine;

    public const int TILE_PRIMITIVE_SIZE = 2 * DX11Engine.VERTEX_SIZE * 3;
    ...
    public const int PRIMITIVE_COUNT = SCENE_PRIMITIVE_SIZE / (DX11Engine.VERTEX_SIZE * 3);

    internal VertexBuffer[] layers;

    internal DX11Scene(int id)
        : base(id)
    {
    ...
    protected override void RefreshLayersImpl(int startRow, int startCol, int endRow, int endCol)
    {
        ...
        {
            DX11DataStream downLayerVBData = layers[0].Lock(startPos, sizeToLock, LockFlags.None);
            DX11DataStream upLayerVBData = layers[1].Lock(startPos, sizeToLock, LockFlags.None);
    ...
    {
        layers[0] = new VertexBuffer(DX11Engine.Engine.Device, SCENE_PRIMITIVE_SIZE, Usage.WriteOnly, DX11Engine.D3DFVF_TLVERTEX, Pool.Managed);
        layers[1] = new VertexBuffer(DX11Engine.Engine.Device, SCENE_PRIMITIVE_SIZE, Usage.WriteOnly, DX11Engine.D3DFVF_TLVERTEX, Pool.Managed);

        DX11DataStream downLayerVBData = layers[0].Lock(0, 0, LockFlags.None);
        DX11DataStream upLayerVBData = layers[1].Lock(0, 0, LockFlags.None);
    ...
    {
        Engine.RenderVertexBuffer(layers[layer], DX11Engine.VERTEX_SIZE, PRIMITIVE_COUNT, tilemap, palette, fadingControl, sceneBox);
    }
}