...
public class SpriteCollider
{
    ...
    public bool TouchingLethalSpikeLeft => BlockedLeft && LeftMaskFlags.IsLethalSpike();

    public bool TouchingNonLethalSpikeLeft => BlockedLeft && LeftMaskFlags.IsNonLethalSpike();

    public bool TouchingLethalSpikeUp => BlockedUp && UpMaskFlags.IsLethalSpike();

    public bool TouchingNonLethalSpikeUp => BlockedUp && UpMaskFlags.IsNonLethalSpike();

    public bool TouchingLethalSpikeRight => BlockedRight && RightMaskFlags.IsLethalSpike();

    public bool TouchingNonLethalSpikeRight => BlockedRight && RightMaskFlags.IsNonLethalSpike();

    public bool LandedOnBlock => DownMaskFlags.IsBlock();

    public bool LandedOnLethalSpike => DownMaskFlags.IsLethalSpike() && !DownMaskFlags.IsBlock() && !DownMaskFlags.IsTopLadder();

    public bool LandedOnNonLethalSpike => DownMaskFlags.IsNonLethalSpike() && !DownMaskFlags.IsBlock() && !DownMaskFlags.IsTopLadder();

    public bool LandedOnSlope => DownMaskFlags.IsSlope() && !DownMaskFlags.IsBlock() && !DownMaskFlags.IsTopLadder() && !DownMaskFlags.IsSpike();

    public bool LandedOnTopLadder => DownMaskFlags.IsTopLadder();
    ...
    private void UpdateColliders()
    {
        ...
        LeftCollider = ((box.Left, box.Origin.Y), (0, box.Mins.Y + headHeight - 1), (1, box.Maxs.Y - legsHeight - 1));
        UpCollider = ((box.Origin.X, box.Top - 1), (box.Mins.X, 0), (box.Maxs.X, 1));
        RightCollider = ((box.Right + 1, box.Origin.Y), (-1, box.Mins.Y + headHeight - 1), (0, box.Maxs.Y - legsHeight - 1));
    ...
    public bool MoveContactSolid(Vector dir, FixedSingle maxDistance, Direction masks = Direction.BOTH, CollisionFlags ignore = CollisionFlags.NONE)
    {
        ...
        var delta = delta1.GetLength(Metric.MAX) < delta2.GetLength(Metric.MAX) ? delta1 : delta2;
