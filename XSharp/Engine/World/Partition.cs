    ...
    internal class Partition<T> where T : Entity
    {
        ...
        public void Insert(T item)
        {
            ...
            for (int i = startCol; i <= endCol; i++)
                ...
                {
                    var box1 = new Box(new Vector(mins.X + cellWidth * i, mins.Y + cellHeight * j), Vector.NULL_VECTOR, new Vector(cellWidth, cellHeight));
        ...
        public List<T> Query(Box box) => Query(box, null, null);

        public List<T> Query(Box box, T exclude) => Query(box, exclude, null);

        public List<T> Query(Box box, List<T> exclusionList) => Query(box, null, exclusionList);
