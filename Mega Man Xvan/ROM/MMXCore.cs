...
namespace MMX.ROM
{
    ...
    public class MMXCore : SNESCore
    {
        ...
        private byte type;
        private byte[] vram;
        private ushort[] mapping;
        private byte[] mappingBG;
        private byte[] sceneLayout;

        private uint[] palettesOffset;
        ...
        // checkpoints
        private List<CheckPointInfo> checkpointInfoTable;
        ...
        // graphics to palette
        private Dictionary<uint, uint> graphicsToPalette;
        private Dictionary<uint, uint> graphicsToAssembly;
        ...
        private uint pEvents;
        private uint pBorders;
        private uint pLocks;
        private uint pProperties;
        private uint pGfx;
        private uint pGfxPos;
        private uint pGfxObj;
        private uint pGfxPal;
        private uint pSpriteAssembly;
        private uint[] pSpriteOffset;
        ...
        private uint expandedSceneSize;
        private uint expandedCheckpointSize;
        private uint expandedLayoutScenes;

        // Events
        private List<EventInfo>[] eventTable;

        // Properties
        private PropertyInfo[] propertyTable;

        // SPRITE
        private HashSet<uint> spriteUpdate;
        ...
        internal System.Drawing.Rectangle GetBoundingBox(ref EventInfo e)
        {
                ...
                )
            {
                ...
                System.Drawing.Rectangle boundingBox = new System.Drawing.Rectangle(0, 0, UInt16.MaxValue, UInt16.MaxValue);
        ...
        internal bool ExpandROM()
        {
            ...
            if (type < 3)
            {
                ...
                if (ok)
                {
                    ...
                    Dictionary<uint, uint> nextAddress = new Dictionary<uint, uint>();
                    for (int i = 0; i < numLevels; i++)
                    {
        ...
        internal void SortTiles()
        {
            ...
            List<ESort> sortTypes = new List<ESort>();
            ...
            foreach (ESort sortType in sortTypes)
            {
                ...
                STileInfo[] tileInfo = new STileInfo[0x400];
        ...
        internal void LoadLevel(bool skipEvent = false, bool skipLayout = false)
        {
            ...
            {
                string sFileName = "C:\\Users\\miste\\Documents\\Projects\\c#\\vc#\\Mega Man Xvan\\Mega Man Xvan\\resources\\roms\\" + ROM_NAME + ".smc";
                ...
                sFileName = "C:\\Users\\miste\\Documents\\Projects\\c#\\vc#\\Mega Man Xvan\\Mega Man Xvan\\resources\\roms\\" + ROM_NAME + ".smc";
        ...
        internal void LoadTilesAndPalettes()
        {
            ...
            {
                List<uint> indices = new List<uint>(new uint[] { 0x124, 0x0, 0x1E, 0x1B2, 0xA, 0x104 });
                if (level >= 0)
                {
                    //indices.clear();
        ...
        internal void LoadEvents()
        {
            ...
            else
            {
                ...
                Deque<uint> workQueue = new Deque<uint>(new uint[] { 0x0 });
                HashSet<uint> indexSeen = new HashSet<uint>(new uint[] { 0x0 });
        ...
        internal void LoadProperties()
        {
            ...
            if (type != 2)
            {
                ...
                for (uint i = 0; i < 107; ++i)
                {
                    ...
                    Deque<Tuple<uint, uint, uint>> workQueue = new Deque<Tuple<uint, uint, uint>>();
        ...
        internal void LoadCheckpoints()
        {
            ...
            HashSet<uint> subIds = new HashSet<uint>();
            foreach (var eventList in eventTable)
            {
            ...
            for (int i = 0; i < numCheckpoints; ++i)
            {
                ...
                CheckPointInfo ci = new CheckPointInfo();
