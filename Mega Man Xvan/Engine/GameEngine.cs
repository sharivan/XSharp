...
namespace MMX.Engine
{
    public class GameEngine : IDisposable
    {
        ...
        private World world;
        internal Partition<MMXObject> partition;
        private Player player;
        internal long engineTime;
        internal Random random;
        internal List<MMXObject> objects;
        internal List<MMXObject> addedObjects;
        internal List<MMXObject> removedObjects;
        ...
        private MMXFloat drawScale;

        internal Checkpoint currentCheckpoint;
        internal MMXVector extensionOrigin;
        internal List<MMXVector> extensions;
        ...
        public GameEngine(Form form, SharpDX.Direct2D1.Factory d2dFactory, ImagingFactory imgFactory, SharpDX.DirectWrite.Factory dwFactory, RenderTarget target)
        {
            ...
            partition = new Partition<MMXObject>(world.BoundingBox, world.SceneRowCount, world.SceneColCount);
            cameraConstraintsBox = world.BoundingBox;

            random = new Random();
            objects = new List<MMXObject>();
            addedObjects = new List<MMXObject>();
            removedObjects = new List<MMXObject>();
            ...
            MMXVector normalOffset = new MMXVector(HITBOX_WIDTH / 2, HITBOX_HEIGHT + 3);
            MMXVector dashingOffset = new MMXVector(DASHING_HITBOX_WIDTH / 2, DASHING_HITBOX_HEIGHT + 3);
        ...
        public SharpDX.Direct2D1.Bitmap CreateD2DBitmapFromStream(Stream stream, SharpDX.Color[] customPalette = null)
        {
        ...
        public SharpDX.Direct2D1.Bitmap CreateD2DBitmapFromStream(Stream stream, Guid srcFormat, Guid dstFormat, SharpDX.Color[] customPalette = null)
        {
        ...
        private void OnFrame()
        {
            ...
            if (!paused && !loadingLevel)
            {
                ...
                {
                    foreach (MMXObject added in addedObjects)
                    {
                        objects.Add(added);
                        partition.Insert(added);
                        added.index = objects.Count - 1;
                    }
                ...
                foreach (MMXObject obj in objects)
                {
                    if (changeLevel)
                        break;
                ...
                {
                    foreach (MMXObject removed in removedObjects)
                    {
        ...
        public static Vector2 ToVector2(MMXVector v)
        {
        ...
        public Vector2 TransformVector(MMXVector v)
        {
            return ToVector2((v - world.Screen.LeftTop) * drawScale + drawBox.Origin);
        }

        public Vector2 TransformVector(MMXFloat x, MMXFloat y)
        {
            return TransformVector(new MMXVector(x, y));
        }

        public MMXVector TransformPoint(SharpDX.Point p)
        {
            return (new MMXVector(p.X, p.Y) - drawBox.Origin) / drawScale + world.Screen.LeftTop;
        }

        public MMXVector TransformPoint(Vector2 v)
        {
            return (new MMXVector(v.X, v.Y) - drawBox.Origin) / drawScale + world.Screen.LeftTop;
        }

        public SharpDX.RectangleF TransformBox(MMXBox box)
        {
            return ToRectangleF((box.LeftTopOrigin() - world.Screen.LeftTop) * drawScale + drawBox.Origin);
        }

        public MMXBox TransformRectangle(SharpDX.RectangleF rect)
        {
            return ((new MMXBox(rect.Left, rect.Top, rect.Width, rect.Height) - drawBox.Origin) / drawScale).LeftTopOrigin() + world.Screen.LeftTop;
        }
        ...
        public void ScheduleRespawn(Player player, MMXFloat time)
        {
        ...
        private void SpawnPlayer()
        {
            ...
            if (romLoaded)
            {
                spawnPos = mmx.CharacterPos + MMXVector.DOWN_VECTOR * (HITBOX_HEIGHT + 1);
                cameraPos = mmx.CameraPos;
            }
            else
            {
                spawnPos = new MMXVector(SCREEN_WIDTH * 0.5f, 0);
                cameraPos = new MMXVector(SCREEN_WIDTH * 0.5f, 0);
            }
        ...
        public void LoadLevel(int level)
        {
            ...
            lock (this)
            {
                ...
                world.FillRectangle(new MMXBox(0, world.Height - BLOCK_SIZE, world.Width, BLOCK_SIZE), block);
                world.EndUpdate();
        ...
        {
            foreach (MMXObject obj in addedObjects)
            {
        ...
        {
            MMXFloat width = form.ClientSize.Width;
            MMXFloat height = form.ClientSize.Height;

            MMXVector drawOrigin;
            ...
            drawOrigin = MMXVector.NULL_VECTOR;

            drawBox = new MMXBox(drawOrigin.X, drawOrigin.Y, width, height);
        }

        private void DrawLine(MMXFloat x1, MMXFloat y1, MMXFloat x2, MMXFloat y2, SolidColorBrush brush)
        {
            target.DrawLine(TransformVector(x1, y1), TransformVector(x2, y2), brush);
        }

        private void DrawLine(MMXFloat x1, MMXFloat y1, MMXFloat x2, MMXFloat y2, SolidColorBrush brush, float strokeWidth)
        {
            target.DrawLine(TransformVector(x1, y1), TransformVector(x2, y2), brush, strokeWidth);
        }

        private void DrawLine(MMXVector v1, MMXVector v2, SolidColorBrush brush)
        {
            target.DrawLine(TransformVector(v1), TransformVector(v2), brush);
        }

        private void DrawLine(MMXVector v1, MMXVector v2, SolidColorBrush brush, float strokeWidth)
        {
            target.DrawLine(TransformVector(v1), TransformVector(v2), brush, strokeWidth);
        }

        private void DrawSlopeMap(MMXBox box, MMXRightTriangle triangle, float strokeWidth)
        {
            MMXVector tv1 = triangle.Origin;
            MMXVector tv2 = triangle.HCathetusVertex;
            MMXVector tv3 = triangle.VCathetusVertex;

            DrawLine(tv2, tv3, touchingMapBrush, strokeWidth);

            MMXFloat h = tv1.Y - box.Top;
        ...
        private void CheckAndDrawTouchingMap(int row, int col, CollisionData collisionData, MMXBox collisionBox, bool ignoreSlopes = false)
        {
            ...
            if (World.IsSolidBlock(collisionData) && (mapBox & collisionBox).Area() > 0)
                target.DrawRectangle(TransformBox(mapBox), touchingMapBrush, 4);
            else if (!ignoreSlopes && World.IsSlope(collisionData))
            {
                MMXRightTriangle st = World.MakeSlopeTriangle(collisionData) + mapBox.LeftTop;
        ...
        public void Render()
        {
            ...
            if (world != null)
            {
                ...
                // Desenha os sprites
                List<MMXObject> objects = partition.Query(world.Screen.BoudingBox);
                foreach (MMXObject obj in objects)
                {
                    ...
                    else
                    {
                        ...
                        {
                            target.DrawRectangle(TransformBox(trigger.BoundingBox), triggerBoxBrush, 4);
                            CameraEventTrigger camTrigger = trigger as CameraEventTrigger;
                            if (camTrigger != null)
                            {
                                MMXVector extensionOrigin = camTrigger.ExtensionOrigin;
                                for (int i = 0; i < camTrigger.ExtensionCount; i++)
                                {
                                    MMXVector extension = camTrigger.GetExtension(i);
                                    target.DrawLine(TransformVector(extensionOrigin), TransformVector(extensionOrigin + extension), cameraEventExtensionBrush, 4);
                                }
                            }
                        }
                    }
                ...
                if (DEBUG_DRAW_COLLISION_BOX)
                {
                    ...
                    target.FillRectangle(TransformBox(collisionBox), hitBoxBrush);
                    target.DrawRectangle(TransformBox(collisionBox), hitBoxBorderBrush, 1);
                }

                if (DEBUG_HIGHLIGHT_TOUCHING_TILES)
                {
                    MMXBox collisionBox = player.CollisionBox;

                    CheckAndDrawTouchingMaps(collisionBox + MMXVector.LEFT_VECTOR, true);
                    CheckAndDrawTouchingMaps(collisionBox + MMXVector.UP_VECTOR);
                    CheckAndDrawTouchingMaps(collisionBox + MMXVector.RIGHT_VECTOR, true);
                    CheckAndDrawTouchingMaps(collisionBox + MMXVector.DOWN_VECTOR);
                }

                SharpDX.RectangleF drawRect = RenderRectangle;

                if (DEBUG_DRAW_BOX)
                    target.DrawRectangle(drawRect, screenBoxBrush, 4);

                if (DEBUG_DRAW_PLAYER_ORIGIN_AXIS)
                {
                    Vector2 v = TransformVector(player.Origin);
                    target.DrawLine(new Vector2(v.X, drawRect.Top), new Vector2(v.X, drawRect.Bottom), playerOriginBrush, 1);
                    target.DrawLine(new Vector2(drawRect.Left, v.Y), new Vector2(drawRect.Right, v.Y), playerOriginBrush, 1);
                }

                if (DEBUG_DRAW_CHECKPOINT && currentCheckpoint != null)
                    target.DrawRectangle(TransformBox(currentCheckpoint.BoundingBox), checkpointBoxBrush, 4);

                if (DEBUG_SHOW_INFO_TEXT)
                    target.DrawText(string.Format("X: {0} Y: {1} VX: {2} VY: {3} Checkpoint: {4}", (int) (player.Origin.X * 256), (int) ((player.Origin.Y - 17) * 256), (int) (player.Velocity.X * 256), (int) (player.Velocity.Y * -256), currentCheckpoint != null ? currentCheckpoint.Index.ToString() : "none"), coordsTextFormat, new SharpDX.RectangleF(drawRect.Left, drawRect.Bottom - 50, drawRect.Width, 50), coordsTextBrush);
            }
        ...
        public CollisionFlags GetCollisionFlags(MMXBox collisionBox, out MMXRightTriangle slope, CollisionFlags ignore = CollisionFlags.NONE, bool preciseCollisionCheck = true, CollisionSide side = CollisionSide.INNER)
        {
        ...
        public CollisionFlags GetCollisionFlags(MMXBox collisionBox, List<CollisionPlacement> placements, out MMXRightTriangle slope, CollisionFlags ignore = CollisionFlags.NONE, bool preciseCollisionCheck = true, CollisionSide side = CollisionSide.INNER)
        {
        ...
        private void LoadState(BinaryReader reader)
        {
            ...
            currentCheckpoint = checkPointIndex != -1 ? (Checkpoint) objects[checkPointIndex] : null;
            ...
            World.Screen.Center = new MMXVector(reader);
            int focusedObjectIndex = reader.ReadInt32();
            World.Screen.FocusOn = objects[focusedObjectIndex];
        ...
        public Checkpoint AddCheckpoint(MMXBox boundingBox, MMXVector characterPos, MMXVector cameraPos, MMXVector backgroundPos, MMXVector forceBackground, uint scroll)
        {
            Checkpoint checkpoint = new Checkpoint(this, boundingBox, characterPos, cameraPos, backgroundPos, forceBackground, scroll);
            addedObjects.Add(checkpoint);
            return checkpoint;
        }

        public CameraEventTrigger AddCameraEventTrigger(MMXBox boundingBox, IEnumerable<MMXVector> extensions)
        {
            CameraEventTrigger trigger = new CameraEventTrigger(this, boundingBox, extensions);
            addedObjects.Add(trigger);
            return trigger;
        }

        public void SetExtensions(MMXVector origin, IEnumerable<MMXVector> extensions)
        {
