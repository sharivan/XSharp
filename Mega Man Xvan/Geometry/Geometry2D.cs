...
namespace Geometry2D
{
    ...
    public struct Interval
    {
        ...
        private Interval(float min, bool closedLeft, float max, bool closedRight)
        {
            this.min = min;
        ...
        public bool Contains(float element, bool inclusive = true)
        {
            if (!inclusive)
        ...
        public static Interval MakeOpenInterval(float v1, float v2)
        {
            return new Interval(Math.Min(v1, v2), false, Math.Max(v1, v2), false);
        }

        public static Interval MakeClosedInterval(float v1, float v2)
        {
            return new Interval(Math.Min(v1, v2), true, Math.Max(v1, v2), true);
        }

        public static Interval MakeSemiOpenLeftInterval(float v1, float v2)
        {
            return new Interval(Math.Min(v1, v2), false, Math.Max(v1, v2), true);
        }

        public static Interval MakeSemiOpenRightInterval(float v1, float v2)
        {
    ...
    public struct Box2D : Shape2D
    {
        ...
        public Vector2D LeftMiddle
        {
            get
            {
                return new Vector2D(Left, (Top + Bottom) / 2);
            }
        }
        ...
        public Vector2D RightMiddle
        {
            get
            {
                return new Vector2D(Right, (Top + Bottom) / 2);
            }
        }

        public Vector2D MiddleTop
        {
            get
            {
                return new Vector2D((Left + Right) / 2, Top);
            }
        }

        public Vector2D MiddleBottom
        {
            get
            {
                return new Vector2D((Left + Right) / 2, Bottom);
            }
        }
        ...
        {
            return new Box2D(LeftTop, Vector2D.NULL_VECTOR, DiagonalVector);
        }

        public Box2D RightBottomOrigin()
        {
            return new Box2D(RightBottom, -DiagonalVector, Vector2D.NULL_VECTOR);
        }

        public Box2D CenterOrigin()
        {
            Vector2D sv2 = DiagonalVector / 2;
        ...
        public Box2D ClipLeft(float clip)
        {
            return new Box2D(Origin, new Vector2D(mins.X + clip, mins.Y), maxs);
        }
        ...
        public Box2D ClipRight(float clip)
        {
            return new Box2D(Origin, mins, new Vector2D(maxs.X - clip, maxs.Y));
        }
        ...
        public Box2D ClipTop(float clip)
        {
            return new Box2D(Origin, new Vector2D(mins.X, mins.Y + clip), maxs);
        }
        ...
        public Box2D ClipBottom(float clip)
        {
            return new Box2D(Origin, mins, new Vector2D(maxs.X, maxs.Y - clip));
        }
        ...
        public Vector2D GetNormal(BoxSide side)
        {
            switch (side)
            {
                case BoxSide.LEFT:
        ...
        public Box2D HalfLeft()
        {
            return new Box2D(origin, mins, new Vector2D((mins.X + maxs.X) / 2, maxs.Y));
        }

        public Box2D HalfTop()
        {
            return new Box2D(origin, mins, new Vector2D(maxs.X, (mins.Y + maxs.Y) / 2));
        }

        public Box2D HalfRight()
        {
            return new Box2D(origin, new Vector2D((mins.X + maxs.X) / 2, mins.Y), maxs);
        }

        public Box2D HalfBottom()
        {
            return new Box2D(origin, new Vector2D(mins.X, (mins.Y + maxs.Y) / 2), maxs);
        }
    ...
    public struct RightTriangle2D : Shape2D
    {
        public static readonly RightTriangle2D EMPTY = new RightTriangle2D(Vector2D.NULL_VECTOR, 0, 0);

        private Vector2D origin;
        private float hCathetus;
        private float vCathetus;
        ...
        public Vector2D HCathetusVertex
        {
            get
            {
        ...
        public Vector2D VCathetusVertex
        {
            get
            {
        ...
        public float HCathetus
        {
            get
            {
                return Math.Abs(hCathetus);
            }
        }

        public float VCathetus
        {
            get
            {
                return Math.Abs(vCathetus);
            }
        }
        ...
        public float Hypotenuse
        {
            get
            {
                return (float) Math.Sqrt(hCathetus * hCathetus + vCathetus * vCathetus);
            }
        }

        public Vector2D HCathetusVector
        {
            get
            {
                return new Vector2D(hCathetus, 0);
            }
        }

        public Vector2D VCathetusVector
        {
            get
            {
                return new Vector2D(0, vCathetus);
            }
        }

        public Vector2D HypotenuseVector
        {
            get
            {
        ...
        public Box2D WrappingBox
        {
            get
            {
                return new Box2D(Math.Min(origin.X, origin.X + hCathetus), Math.Min(origin.Y, origin.Y + vCathetus), HCathetus, VCathetus);
            }
        }
        ...
        public int VCathetusSign
        {
            get
            {
                return Math.Sign(vCathetus);
            }
        }

        public RightTriangle2D(Vector2D origin, float hCathetus, float vCathetus)
        {
            this.origin = origin;
        ...
        public RightTriangle2D Translate(Vector2D shift)
        {
            return new RightTriangle2D(origin + shift, hCathetus, vCathetus);
        }
        ...
        public Vector2D GetNormal(RightTriangleSide side)
        {
            switch (side)
            {
                case RightTriangleSide.HCATHETUS:
                    return vCathetus >= 0 ? Vector2D.UP_VECTOR : Vector2D.DOWN_VECTOR;

                case RightTriangleSide.VCATHETUS:
                    return hCathetus >= 0 ? Vector2D.RIGHT_VECTOR : Vector2D.LEFT_VECTOR;

                case RightTriangleSide.HYPOTENUSE:
                {
                    int sign = Math.Sign(vCathetus) == Math.Sign(hCathetus) ? 1 : -1;
                    Vector2D v = new Vector2D(sign * vCathetus, -sign * hCathetus);
                    return v.Versor();
                }
            }
        ...
        private static bool PointInTriangle(Vector2D pt, Vector2D v1, Vector2D v2, Vector2D v3)
        {
            float d1, d2, d3;
        ...
        public bool Contains(Vector2D v, bool inclusive = true, bool excludeHypotenuse = false)
        {
        ...
        public bool HasIntersectionWith(Box2D box, bool excludeHypotenuse = false)
        {
            Box2D intersection = box & WrappingBox;
        ...
        public static bool operator ==(RightTriangle2D left, RightTriangle2D right)
        {
        ...
        public static RightTriangle2D operator +(RightTriangle2D triangle, Vector2D shift)
        {
    ...
    /// </summary>
    public struct Polygon2D : Shape2D
    {
