...
public class DX9DataStream : DataStream
{
    public unsafe static DX9DataStream Create<T>(T[] userBuffer, bool canRead, bool canWrite, int index = 0, bool pinBuffer = true) where T : struct
    {
    ...
    public DX9DataStream(DataStreamImpl impl, bool canDispose = true)
    {
        this.impl = impl;
        this.canDispose = canDispose;
    }

    public unsafe DX9DataStream(Blob buffer)
    {
        impl = new DataStreamImpl(buffer);
        canDispose = true;
    }

    public unsafe DX9DataStream(int sizeInBytes, bool canRead, bool canWrite)
    {
        impl = new DataStreamImpl(sizeInBytes, canRead, canWrite);
        canDispose = true;
    }

    public DX9DataStream(DataPointer dataPointer)
    {
        impl = new DataStreamImpl(dataPointer);
        canDispose = true;
    }

    public unsafe DX9DataStream(IntPtr userBuffer, long sizeInBytes, bool canRead, bool canWrite)
    {
    ...
    public static implicit operator DataPointer(DX9DataStream from)
    {
        return new DataPointer(from.PositionPointer, (int) from.RemainingLength);
    }

    public static implicit operator DataStreamImpl(DX9DataStream from)
    {
        return from?.impl;
    }

    public static implicit operator DX9DataStream(DataStreamImpl from)
    {
        return new DX9DataStream(from);
    }
}